...
INSERT INTO Invoices (InvoiceID, VendorID, InvoiceNumber, InvoiceDate, InvoiceTotal, PaymentTotal, CreditTotal,
					  TermsID, InvoiceDueDate, PaymentDate, isDeleted)
	...
	FROM (
		SELECT 	[id] = child.value('@id', 'int'),
				...
				[invNum] = child.value('@invNum', 'varchar(50)'),
				[invDate] = child.value('@invDate', 'smalldatetime'),
				[invTot] = child.value('@invTot', 'money'),
				[paymentTot] = child.value('@paymentTot', 'money'),
				[creditTot] = child.value('@creditTot', 'money'),
				[termsId] = child.value('@termsId', 'int'),
				[invDueDate] = child.value('@invDueDate', 'smalldatetime'),
				[paymentDate] = child.value('@paymentDate', 'smalldatetime'),
				[isDeleted] = child.value('@isDeleted', 'bit')
		FROM @xml.nodes('AP-CRUD/CREATE/invoice') Child(child)
	) tbl
...
SELECT	v.VendorID,
		...
		v.VendorCity,
		v.VendorState,
		--[id] = parent.value('@id', 'int'),
		[comments] = CAST(parent.query('comments/text()') AS VARCHAR(100)),
		[recommendations] = CAST(parent.query('recommendation/text()') AS VARCHAR(200))
FROM @xml.nodes('AP-CRUD/READ/vendor') Parent(parent)
	JOIN Vendors v ON v.VendorID = parent.value('@id', 'int')
ORDER BY v.VendorID
...
UPDATE Terms
SET
	TermsDescription 	= ISNULL(child.value('@desc', 'varchar(50)'), TermsDescription),
...
UPDATE Vendors
SET 
	VendorName = ISNULL(child.value('@VendorName', 'varchar(50)'), VendorName)
...
DELETE FROM InvoiceLineItems
	FROM @xml.nodes('AP_CRUD/DELETE/invoiceLineItems') Child(child)
WHERE InvoiceID = child.value('@InvoiceID', 'int')
