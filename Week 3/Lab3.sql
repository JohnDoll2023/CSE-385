...
SELECT TOP(5) VendorID,
       [TotalInvoices] = SUM(InvoiceTotal),
       [InvoiceCount] = COUNT(*)
FROM Invoices
GROUP BY VendorID
...
SELECT VendorName, 
       ...
       VendorZipCode
FROM Vendors
WHERE VendorPhone LIKE '([8][0][0]%' AND VendorZipCode LIKE '[1-5][3-5]%[5]'
...
SELECT VendorName, 
       ...
       VendorPhone
FROM Vendors
WHERE (VendorAddress1 LIKE '%Box%' OR VendorAddress2 LIKE '%Box%') AND (VendorPhone IS NOT NULL)
ORDER BY VendorName